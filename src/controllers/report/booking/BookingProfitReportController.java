...
public class BookingProfitReportController extends Controller {
    private Log logger = Log.getLog(BookingProfitReportController.class);
    ...
    public  long list() {
        ...
        if(StringUtils.isNotEmpty(customer_name)){
        	condition += " and p.abbr = '"+customer_name+"'";
        }
        ...
        if(StringUtils.isBlank(group_condition)){
        	group_condition = "jo.";
        }
        
        if(StringUtils.isNotBlank(begin_date)||StringUtils.isNotBlank(end_date)){
        	condition += " and jo.order_export_date between '"+begin_date+" 00:00:00' and '"+end_date+" 23:59:59' "; 
        }else{
        	condition += " and jo.order_export_date between '1970-1-1 00:00:00' and '2037-12-31 23:59:59'";
        }
  ...
  /*      String sql = "  SELECT "
	...
	public void downloadExcelList() {
		...
		String sql = "select B.* "
				...
				// + " ) AS char) truck_type "
				// //get_truck_type(B.truck_order_ids)''
				+ " from (select order_export_date,customer_id,"
				+ " sum(ifnull(pieces,0)) pieces,"
				+ " sum(ifnull(gross_weight,0)) gross_weight, "
				+ " sum(ifnull(volume,0)) volume, "
				...
				+ " from (" + " select "
				+ group_condition
				+ " order_export_date, jo.customer_id, jo.pieces,"
				+ " jo.gross_weight, jo.volume,p.abbr customer_name,"
				+ " (select ("
				+ "    count(case when container_type = '20''GP' then container_type end) +"
				+ "    count(case when container_type = '40''GP' then container_type end)*2 +"
		...
		String[] fields = { "ORDER_EXPORT_DATE", "CUSTOMER_NAME", "PIECES",
				"GROSS_WEIGHT", "VOLUME", "OCEAN_FCL_TEU", "OCEAN_FCL_BILL",
				"OCEAN_LCL_CBM", "OCEAN_LCL_BILL", "ARI_KG", "ARI_KG_BILL",
				"TRUCK_TYPE" };
