...
public class TradeCostCheckOrderController extends Controller {
	private Log logger = Log.getLog(TradeCostCheckOrderController.class);
	...
	public void index() {
		...
		render("/tradeArap/CostCheckOrder/CostCheckOrder.html");
	}
        ...
        if(checked!=null&&!"".equals(checked)&&checked.equals("Y")){
        	sql = "select * from(  "
        			+ " select joa.order_type sql_type, joa.id,joa.type,joa.sp_id,ifnull(joa.total_amount,0) total_amount,ifnull(joa.currency_total_amount,0) currency_total_amount,"
              		...
              		+ " p.abbr sp_name,p1.abbr customer_name,'' mbl_no, '' hbl_no, '' fnd, '' destination, "
              		+ " '' container_no, '' container_amount, "
              		+ " ifnull(cur.name,'CNY') currency_name,'' truck_type ,ifnull(joa.exchange_rate,1) exchange_rate,"
