...
public class TradeAccountAuditLogController extends Controller {
    ...
    public void index() {
    	...
    	List<TradeArapAccountAuditLog> accountlist = TradeArapAccountAuditLog.dao.find("SELECT DISTINCT a.bank_name FROM trade_arap_account_audit_log aaa left join fin_account a on a.id = aaa.account_id");
...
//        }
        ...
        if(true){
        	sql = " select * from (select aaal.*,ifnull(ul.c_name, ul.user_name) user_name, fa.bank_name,"
				    ...
				    + " THEN "
				    + " (select aco.order_no FROM trade_arap_charge_application_order aco where aco.id = aaal.invoice_order_id )"
				    + " WHEN aaal.source_order = '应付申请单' "
				    + " THEN "
				    + " (select aco.order_no FROM trade_arap_cost_application_order aco where aco.id = aaal.invoice_order_id )"
				    + " end ) order_no,"
				    + " (select amount from trade_arap_account_audit_log where id=aaal.id and payment_type='cost') cost_amount,"
				    + " (select amount from trade_arap_account_audit_log where id=aaal.id and payment_type='charge') charge_amount"
				    + " from trade_arap_account_audit_log aaal"
    ...
    public void accountList() {
    	...
    	String sql = " SELECT fa.office_id,fa.id,(select bank_name from fin_account where id = fa.id) bank_name,'"+ beginTime +"' date, "
    			...
    			+ " FROM trade_arap_account_audit_log aa"
    			...
    			+ "  - "
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2) FROM trade_arap_account_audit_log aa  "
    			...
    			+ " ) ) balance_amount"
    			+ " FROM trade_arap_account_audit_log aal"
