...
public class FeeBatchInputController extends Controller {

	private Logger logger = Logger.getLogger(FeeBatchInputController.class);
	...
	public void index() {
		...
		render("/cms/feeBatchInput/FeeBatchInputlist.html");
	}
	...
	public void list() {
        ...
        if("unConfirmFee".equals(confirmFee)){
        	sql = "SELECT * from (SELECT cpo.id,cpo.to_office_id,cpo.order_no,cpo.date_custom,cpo.tracking_no,cpo.customs_billCode,cpo.type,cpo.receive_sent_consignee_input receive_company_name,ul.c_name creator_name,cpo.booking_no,"
        			...
        			+ " (SELECT COUNT(3) from custom_plan_order cpo WHERE cpo.custom_state = '异常' and (cpo.office_id="+office_id+" or cpo.to_office_id ="+office_id+") AND cpo.delete_flag = 'N') abnormal,"
        			+ " (SELECT COUNT(4) from custom_plan_order cpo WHERE cpo.status = '待审核' and (cpo.office_id="+office_id+" or cpo.to_office_id ="+office_id+") AND cpo.delete_flag = 'N') waitAuditing,"
