...
public class CustomAccountAuditLogController extends Controller {
    private Log logger = Log.getLog(CustomAccountAuditLogController.class);
...
//        }
    ...
    public void accountList() {
    	...
    	String sql = " SELECT fa.office_id,fa.id,(select bank_name from fin_account where id = fa.id) bank_name,'"+ beginTime +"' date, "
    			...
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code = aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+(month-1)+"-31 23:59:59' AND aa.payment_type = 'CUSTOMCHARGE' ) "
    			+ " - "
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa"
    			+ " WHERE account_id = fa.id AND aa.currency_code = aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+(month-1)+"-31 23:59:59' AND aa.payment_type = 'CUSTOMCOST'"
    			+ " ) ) init_amount,"
    			+ " ( ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa "
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code = aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '"+year+"-"+month+"-01' AND '"+year+"-"+month+"-31 23:59:59' AND aa.payment_type = 'CUSTOMCHARGE'"
    			+ " ) ) total_charge,"
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa"
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code = aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '"+year+"-"+month+"-01' AND '"+year+"-"+month+"-31 23:59:59' AND aa.payment_type = 'CUSTOMCOST'"
    			+ " ) total_cost,"
    			+ " ( ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa"
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code = aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+month+"-31 23:59:59' AND aa.payment_type = 'CUSTOMCHARGE'  )"
    			+ "  - "
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2) FROM arap_account_audit_log aa  "
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code = aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+month+"-31 23:59:59' AND aa.payment_type = 'CUSTOMCOST'"
    			+ " ) ) balance_amount"
