        ...
        if(checked!=null&&!"".equals(checked)&&checked.equals("Y")){
        	 sql = "select * from(  "
        			+ " select joa.order_type sql_type,joa.remark, joa.id,joa.sp_id,ifnull(joa.total_amount,0) total_amount,ifnull(joa.currency_total_amount,0) currency_total_amount,"
              		...
              		+ " after_rate_total,ifnull(f.name,f.name_eng) fee_name,cur1.name exchange_currency_name,joa.exchange_currency_rate,joa.exchange_total_amount,joli.cabinet_date"
        		 ...
        		 sql = "select * from(  "
                 		...
                 		+ " ifnull( ( SELECT rc.new_rate FROM rate_contrast rc "
                 		+ " WHERE rc.currency_id = joa.currency_id AND rc.order_id = '' ), ifnull(joa.exchange_rate, 1) ) * ifnull(joa.total_amount, 0)"
                 		+ " after_rate_total,cur1.name exchange_currency_name,joa.exchange_currency_rate,joa.exchange_total_amount,joli.cabinet_date"
         				+ " from trans_job_order jo "
         				+ " left join trans_job_order_arap joa on jo.id=joa.order_id "
         				+ " left join party p on p.id=joa.sp_id "
         				+ " left join party p1 on p1.id=jo.customer_id "
...
//        UserLogin user = LoginUserController.getLoginUser(this);
        String exportSql =  " SELECT tjo.id,tjo.order_no,tjo.lading_no lading_no,tjo.type, "
        		 ...
        		 +" 	if(tjoa.charge_id = 173,IFNULL(convert(tjoa.total_amount,decimal),''),'') freight, "
        		 +"   cy. NAME  currency_name, "
        		 +" 	if(tjoa.charge_id = 264,IFNULL(convert(tjoa.total_amount,decimal),''),'') weighing_fee, "
        		 +" 	if(tjoa.charge_id = 263,IFNULL(convert(tjoa.total_amount,decimal),''),'') night_fee, "
        		 +" 	if(tjoa.charge_id = 262,IFNULL(convert(tjoa.total_amount,decimal),''),'') high_speed_fee, "
        		 +" 	if(tjoa.charge_id = 174,IFNULL(convert(tjoa.total_amount,decimal),''),'') call_fee, "
        		 +" 	if(tjoa.charge_id = 302,IFNULL(convert(tjoa.total_amount,decimal),''),'') empty_in_out_fee, "
        		 +" 	if(tjoa.charge_id = 497,IFNULL(convert(tjoa.total_amount,decimal),''),'') advance_fee, "
        		 +" 	tjo.remark "
