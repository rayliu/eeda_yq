...
public class DispatchSendController extends Controller {
	ParentOfficeModel pom = ParentOffice.getInstance().getOfficeId(this);
	private Logger logger = Logger.getLogger(DispatchSendController.class);
	...
	public void index() {
		...
		setAttr("type",type);
		render("/tms/Dispatch/dispatchSendList.html");
	}
...
//      SimpleDateFormat parseFormat = new SimpleDateFormat("yyyy-MM-dd");//分析日期
		...
		ArrayList<String> carId_list = new ArrayList<String>();
        ...
        if("sowait".equals(type)){
        	sql=" ";        	
		         ...
		         sql = "SELECT * from (select GROUP_CONCAT(cast(substring(tjol.cabinet_date, 1, 10) AS CHAR)) cabinet_date,"
		         		...
		         		+ " (CASE "
		         		+ "	WHEN tjol.closing_date is NOT NULL then '已收重'"
		         		+ "	WHEN tjol.arrival_date is NOT NULL then '吉柜已到厂'"
		         		+ "	WHEN tjol.cabinet_date is NOT NULL then '已提吉'"
		         		+ "	WHEN tjol.dispatch_date is NOT NULL then '已派车'"
		         		+ "	else '未处理' end) land_dispatch_status, "
