    ...
    private List<Record> getItems(String orderId,String type) {
    	...
    	List<Record> itemList = null;
    	 if("land".equals(type)){
    		...
    		itemSql = " select tjol.*,ci.car_no car_no_name,d1.dock_name take_address_name,d2.dock_name delivery_address_name,IFNULL(IFNULL(d1.dock_name, l1. NAME),p1.address) loading_wharf1_name,d4.dock_name loading_wharf2_name,"
    				...
    				+ " left join dockinfo d3 ON (d3.id = tjol.loading_wharf1 AND tjol.loading_wharf1_type = 'land')"
        ...
        if("sowait".equals(type)){
        	sql=" ";        	
		         ...
		         sql = "SELECT * from (select GROUP_CONCAT(cast(substring(tjol.cabinet_date, 1, 10) AS CHAR)) cabinet_date,"
		         		+ " tjo.create_stamp create_stamp,tjo.order_no,tjo.type,tjo.cabinet_type,tjol.truck_type,tjo.container_no,tjo.so_no,tjo.head_carrier,tjo.id,tjo.from_order_id,tjo.update_apply_flag,tjo.take_wharf,p.charge_company_id sp_id,"
