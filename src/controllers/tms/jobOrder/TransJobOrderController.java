        ...
        if("sowait".equals(type)){
        	sql=" SELECT jor.*,ifnull(u.c_name, u.user_name) creator_name,p.abbr customer_name, ifnull(jos.export_date,joa.export_date) sent_out_time"
		         ...
		         sql = "SELECT * from (select tjol.cabinet_date  cabinet_date,"
						...
						+ " (SELECT GROUP_CONCAT(CONCAT(fi.name,':',tjoa.currency_total_amount)) from trans_job_order_arap tjoa"
						+ " LEFT JOIN fin_item fi on fi.id = tjoa.charge_id "
						+ " WHERE tjoa.order_id=tjo.id and tjoa.order_type='cost'  group by tjoa.order_type ) cost, "
						+ " (SELECT GROUP_CONCAT(CONCAT(fi.name,':',tjoa.currency_total_amount)) from trans_job_order_arap tjoa"
						+ " LEFT JOIN fin_item fi on fi.id = tjoa.charge_id "
						+ " WHERE tjoa.order_id=tjo.id and tjoa.order_type='charge' and fi.name!='运费' group by tjoa.order_type) charge, "
