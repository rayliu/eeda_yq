...
public class RegisterUserController  extends Controller{
	...
	public void index(){
		render("/eeda/profile/registerUser/register.html");
	}
	...
	public void saveRegistrant(){
		...
		//判断密码一致
        if(!pwd.equals(againPassword)){
            Record errRec = new Record();
            errRec.set("status", "failed");
            errRec.set("msg", "输入的密码不一致。");
            setAttr("err", errRec);
            renderError(500);
            return;
        }
		//判断登陆mobile不能重复注册
		UserLogin oldUser = UserLogin.dao.findFirst(
                "select * from user_login where user_phone=?", mobile);
        if(oldUser != null){
            Record errRec = new Record();
            errRec.set("status", "failed");
        ...
        Record rec = Db.findFirst(
                "select * from user_register where mobile=? order by create_date desc", mobile);
        if(rec != null){
    ...
    private void notifyAdmin(String mobile) throws EmailException {
        ...
        String basePath =newDate + " 用户: " + mobile + "simplev 注册了新账号";
