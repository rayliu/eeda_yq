...
//    	Record rec = Db.findFirst(sqlTotal, office_id);
    	...
    	String sql = "SELECT * FROM "
    			...
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code=aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+(month-1)+"-31 23:59:59' AND (aa.payment_type = 'CHARGE' or aa.payment_type ='CUSTOMCHARGE' or aa.payment_type ='TRANSCHARGE') ) "
    			+ " - "
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa"
    			+ " WHERE account_id = fa.id AND aa.currency_code=aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+(month-1)+"-31 23:59:59' AND (aa.payment_type = 'CHARGE' or aa.payment_type ='CUSTOMCOST' or aa.payment_type ='TRANSCOST')"
    			+ " ) ) init_amount,"
    			+ " ( ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa "
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code=aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '"+year+"-"+month+"-01' AND '"+year+"-"+month+"-31 23:59:59' AND (aa.payment_type = 'CHARGE' or aa.payment_type ='CUSTOMCHARGE' or aa.payment_type ='TRANSCHARGE')"
    			+ " ) ) total_charge,"
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa"
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code=aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '"+year+"-"+month+"-01' AND '"+year+"-"+month+"-31 23:59:59' AND (aa.payment_type = 'CHARGE' or aa.payment_type ='CUSTOMCOST' or aa.payment_type ='TRANSCOST')"
    			+ " ) total_cost,"
    			+ " ( ( SELECT ROUND(ifnull(sum(amount), 0), 2)"
    			+ " FROM arap_account_audit_log aa"
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code=aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+month+"-31 23:59:59' AND (aa.payment_type = 'CHARGE' or aa.payment_type ='CUSTOMCHARGE' or aa.payment_type ='TRANSCHARGE')  )"
    			+ "  - "
    			+ " ( SELECT ROUND(ifnull(sum(amount), 0), 2) FROM arap_account_audit_log aa  "
    			+ " WHERE aa.account_id = fa.id AND aa.currency_code=aal.currency_code and aa.office_id = "+office_id+" AND aa.create_date BETWEEN '2015-01-01' AND '"+year+"-"+month+"-31 23:59:59' AND (aa.payment_type = 'CHARGE' or aa.payment_type ='CUSTOMCOST' or aa.payment_type ='TRANSCOST')"
    			+ " ) ) balance_amount"
