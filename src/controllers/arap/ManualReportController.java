...
public class ManualReportController extends Controller {
	...
	public void list() {
        ...
        String sql = "select jor.id,jor.order_no ,jor.customer_id,p.abbr customer_name,jor.order_export_date export_date,ul.c_name creator_name,"
        		...
        		+ " if(joa.order_type='cost',ifnull(joa.total_amount,0),0) cost_check_amount,if(joa.order_type='cost',ifnull(joa.currency_total_amount,0),0) cost_total_amount,"
        		+ " if(joa.order_type='charge',ifnull(joa.total_amount,0),0) charge_check_amount,if(joa.order_type='charge',ifnull(joa.currency_total_amount,0),0) charge_total_amount,"
        		+ " jos.booking_agent,"
        ...
        if(StringUtils.isNotBlank(summary)){
        	...
        	sql = "select group_concat(distinct id separator '<br/>') id,"
        			...
        			+ " ifnull(sum(cost_check_amount),0) cost_check_amount,"
        			+ " ifnull(sum(cost_total_amount),0) cost_total_amount,"
        			+ " ifnull(sum(charge_check_amount),0) charge_check_amount,"
        			+ " ifnull(sum(charge_total_amount),0) charge_total_amount,"
