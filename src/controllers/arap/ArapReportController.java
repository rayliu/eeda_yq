...
public class ArapReportController extends Controller {
	...
	public String list() {
        ...
        String sql =  " SELECT * from ( "
                ...
                +"       joa.charge_id fin_item_id, "
                +"       ( "
                +"         CASE "
                +"         WHEN joa.pay_flag = 'Y' THEN "
                +"           '费用已结算' "
                +"         WHEN joa.create_flag = 'Y' THEN "
                +"           '费用已创建申请单' "
                +"         WHEN joa.billConfirm_flag = 'Y' THEN "
                +"           '费用已创建对账单并已确认' "
                +"         WHEN joa.bill_flag = 'Y' THEN "
                +"           '费用已创建对账单' "
                +"         WHEN joa.audit_flag = 'Y' THEN "
                +"           '费用已确认' "
                +"         ELSE "
                +"           '费用未确认' "
                +"         END "
                ...
                +"       IFNULL(aoCharge.order_no,aoCost.order_no) check_order_no, "
                +"       IFNULL(caoCharge.order_no,caoCost.order_no) application_order_no, "
                + "      aoCharge.id charge_order_id,aoCost.id cost_order_id, "
                + "      caoCharge.id charge_application_order_id,caoCost.id cost_charge_application_order_id"
                +"     FROM "
                +"       job_order_arap joa "
                ...
                +"  "
                +"     LEFT JOIN arap_charge_item aCharge on aCharge.ref_order_id = joa.id "
                +"     LEFT JOIN arap_charge_order aoCharge on aoCharge.id = aCharge.charge_order_id "
                +"  "
                +"     LEFT JOIN arap_cost_item aCost on aCost.ref_order_id = joa.id "
                +"     LEFT JOIN arap_cost_order aoCost on aoCost.id = aCost.cost_order_id "
                +"  "
                +"     LEFT JOIN charge_application_order_rel caCharge on caCharge.job_order_arap_id = joa.id "
                +"     LEFT JOIN arap_charge_application_order caoCharge on caoCharge.id = caCharge.application_order_id "
                +"  "
                +"     LEFT JOIN cost_application_order_rel caCost on caCost.job_order_arap_id = joa.id "
                +"     LEFT JOIN arap_cost_application_order caoCost on caoCost.id = caCost.application_order_id "
                +"      "
                +"     WHERE "
                +"      (jo.office_id = "+office_id + ref_office+")"
