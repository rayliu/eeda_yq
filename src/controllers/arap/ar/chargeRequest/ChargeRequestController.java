...
public class ChargeRequestController extends Controller {
    ...
    public void OrderList() {
        ...
        String sql = " SELECT "
        		...
        		+" WHERE "
        		+" 	( "
        		+" 		((ifnull(usd, 0)-paid_usd > 0.02) or (ifnull(usd, 0)-paid_usd < -0.02)) "
        		+" 		or ((ifnull(cny, 0)-paid_cny > 0.02) or (ifnull(cny, 0)-paid_cny < -0.02)) "
        		+" 		or ((ifnull(hkd, 0)-paid_hkd > 0.01) or (ifnull(hkd, 0)-paid_hkd < -0.01)) "
        		+" 		or ((ifnull(jpy, 0)-paid_jpy > 0.01) or (ifnull(jpy, 0)-paid_jpy < -0.01)) "
        		+" 	) ";
