...
$(document).ready(function() {
    ...
    $("#doc_table").on('click', '.delete', function(e){
        ...
        var id = tr.attr('id');
         ...
         $.post('/costAcceptOrder/deleteDoc', {docId:id}, function(data){
        	 if(data.result==true){
    ...
    itemOrder.refleshDocTable = function(order_id){
    	var url = "/costAcceptOrder/tableList?order_id="+order_id;
    	docTable.ajax.url(url).load();
    }
    
    //上传图片
    $('#fileupload').click(function(){
		var order_id = $('#application_id').val();
		...
		$('#fileupload').fileupload({
				...
				autoUpload: true, 
			    url: '/costAcceptOrder/saveDocFile?order_id='+order_id,
			    dataType: 'json',
