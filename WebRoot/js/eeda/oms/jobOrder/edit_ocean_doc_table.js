...
$(document).ready(function() {
	...
	//未保存不能上传文件
	if($('#order_id').val()==''){
		$("#fileuploadSpanOfOcean").hide();
	}
	
	//上传
	$('#fileuploadOfOcean').click(function(){
		var order_id = $('#order_id').val();
		$('#fileuploadOfOcean').fileupload({
				autoUpload: true, 
			    url: '/jobOrder/uploadOceanDoc?order_id='+order_id,
			    dataType: 'json',
		        done: function (e, data) {
	        	    if(data.result.result){
			    		$.scojs_message('上传成功', $.scojs_message.TYPE_OK);
			    		//异步刷新显示上传的文档信息
			    		itemOrder.refleshOceanDocTable(order_id);
			    	}else{
    ...
    //删除一行
    $("#ocean_doc_table").on('click', '.delete', function(e){
        e.preventDefault();
        var tr = $(this).parent().parent();
        var id = tr.attr('id');
        
         $.post('/jobOrder/deleteOceanDoc', {id:id}, function(data){
        	 if(data.result==true){
        		 oceanDocTable.row(tr).remove().draw();
    ...
    //------------事件处理,文档table
    var oceanDocTable = eeda.dt({
        id: 'ocean_doc_table',
        autoWidth: false,
        columns:[
			{ "data":"ID","width": "80px",
			    "render": function ( data, type, full, meta ) {
                    if(full.REF_JOB_ORDER_ID){//有doc 没有REF_JOB_ORDER_ID证明是自己上传的，否则是从job order 共享过来的
                        return '';
                    }else{
    ...
    $('#ocean_doc_table').on('change','.checkBox',function(){
    ...
    //刷新明细表
    itemOrder.refleshOceanDocTable = function(order_id){
    	var url = "/jobOrder/tableList?order_id="+order_id+"&type=ocean_doc";
    	oceanDocTable.ajax.url(url).load();
    };

    
    //全选
    $('#allCheckOfOceanDoc').click(function(){
