define(['jquery', 'metisMenu', 'template', 'sb_admin',  'dataTablesBootstrap', 'validate_cn', 'sco'], function ($, metisMenu, template) {
    $(document).ready(function() {
    	...
    	var deletedTableIds=[];
        //删除一行
        $("#oceanTotal_cargo_table").on('click', '.delete', function(e){
            e.preventDefault();
        ...
        var bindFieldEvent=function(){
        	eeda.bindTableField('oceanTotal_cargo_table','UNIT_ID','/serviceProvider/searchUnit','');
        };
        //------------事件处理
    	var cargoTable = eeda.dt({
    	    id: 'oceanTotal_cargo_table',
    	    autoWidth: false,
...
//    	if(!boolOcean){
//    		$('#oceanTotal_cargo_table input, #oceanTotal_cargo_table select,#oceanTotal_cargo_table button').prop('disabled', true);
//    		$('#oceanTotal_cargo_table button').css('display', 'none');
//    	}
    	
        $('#add_oceanTotal_cargo').on('click', function(){
            var item={};
        ...
        $('#oceanTotal_cargo_table').on('blur','[name=container_type], [name=vgm]',function(){
        ...
        $('#oceanTotal_cargo_table').on('keyup','[name=vgm]',function(){
        	var vgm = 0;
        	$('#oceanTotal_cargo_table [name=vgm]').each(function(){
        		var val = this.value;
            	if(val!=''&&!isNaN(val)){
            		vgm += parseFloat(val);
            	}
        ...
        if(container_type_hidden!=null && container_type_hidden!=""){
            ...
            var dataTable = $('#oceanTotal_cargo_table').DataTable();
