...
$(document).ready(function() {
    ...
    $('#collapseChargeInfo,#collapseCostInfo').on('show.bs.collapse', function () {
        ...
        if(order_type.trim() == '' || customer_id == ''){
        	...
        	$.post('/bookingOrder/getArapTemplate', {order_type:order_type,customer_id:customer_id,arap_type:type}, function(data){
        		if(data){
    ...
    $('#ChargeDiv,#CostDiv').on('click', '.deleteChargeTemplate,.deleteCostTemplate', function(){
	  	...
	  	var id = ul.attr('id');
	  	$.post('/bookingOrder/deleteArapTemplate', {id:id}, function(data){
	  		if(data){
