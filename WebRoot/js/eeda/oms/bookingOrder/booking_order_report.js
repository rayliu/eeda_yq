		...
		if(alert!=''){
			...
			$('#pdfAlert').click();
		    	...
		    	$.post('/jobOrderReport/printOceanSI', {order_id:order_id}, function(data){
		    		if(data){
		                window.open(data);
		                $.post('/bookingOrder/siflag', {order_id:order_id}, function(data){
...
//			   });
    ...
    $('#oceanMBL').click(function(){
    	var order_id = $("#order_id").val();
    	$.post('/bookingOrder/mblflag',{order_id:order_id},function(data){
    		if(data.result==true){
    ...
    $('#alreadyAFR_AMS').click(function(){
    	var order_id = $("#order_id").val();
    	$.post('/bookingOrder/aframsflag',{order_id:order_id},function(data){
    		if(data.result==true){
    ...
    $('#alreadyInline').click(function(){
    	var order_id = $("#order_id").val();
    	$.post('/bookingOrder/alreadyInlineFlag',{order_id:order_id},function(data){
    		if(data.result==true){
				...
				$('#land_table input[type="checkbox"]').each(function(){
					...
					if(checkbox){
				    	...
				    	$.post('/jobOrderReport/printTruckOrderPDF', {itemId:itemId}, function(data){
							if(data){
								 window.open(data);	
								 $.post('/bookingOrder/truckOrderflag', {itemId:itemId,order_id:order_id}, function(data){
	...
	$('#printDebitNoteBtn').click(function(){
		$(this).attr('disabled', true);
	    	...
	    	var invoiceNo = $('#invoiceNo').val();
	    	 ...
	    	 $.post('/bookingOrder/saveDebitNote', {itemIds:itemIdsStr,invoiceNo:invoiceNo}, function(data){
	    		 if(data.result==true){
	...
	$('#land_printDebit .btn-primary').click(function(){
		...
		$.post('/bookingOrder/saveDebitNoteOfLand', {landIds:landIdsStr,invoice_land_hbl_no:invoice_land_hbl_no,land_ref_no:land_ref_no}, function(data){
			if(data.result==true){
