...
$(document).ready(function() {
	...
	$('#fileuploadOfCustom').click(function(){
		...
		$('#fileuploadOfCustom').fileupload({
				autoUpload: true, 
			    url: '/bookingOrder/uploadCustomDoc?order_id='+order_id,
			    dataType: 'json',
    ...
    $("#custom_doc_table").on('click', '.delete', function(e){
        ...
        var id = tr.attr('id');
         ...
         $.post('/bookingOrder/deleteCustomDoc', {id:id}, function(data){
        	 if(data.result==true){
        ...
        columns:[
			{ "data":"ID","width": "80px",
			    "render": function ( data, type, full, meta ) {
                    if(!full.ID){//有doc id证明是自己上传的，否则是从booking order 共享过来的
                        return '';
                    }else{
    ...
    $('#custom_doc_table').on('change','.checkBox',function(){
    	...
    	$.post('/bookingOrder/updateShare', {order_id:$('#order_id').val(),item_id:item_id,check:check}, function(data){
    		if(data){
    ...
    itemOrder.refleshCustomDocTable = function(order_id){
    	var url = "/bookingOrder/tableList?order_id="+order_id+"&type=custom_doc";
    ...
    $('#allCheckOfCustomDoc').click(function(){
    	...
    	$.post('/bookingOrder/updateShare', {order_id:$('#order_id').val(),check:ischeck}, function(data){
    		if(data){
    			$.scojs_message(msg+'成功', $.scojs_message.TYPE_OK);
    			$.unblockUI();
    		}
