define(['jquery', 'metisMenu', 'sb_admin',  'dataTablesBootstrap', 'validate_cn', 'sco','datetimepicker_CN', 'jq_blockui',
    ...
    './edit_express_detail','./edit_insurance_detail','./edit_party_detail', './edit_doc_table', './edit_file_upload','./book_order_report',
...
$(document).ready(function() {
	...
	var loadOrderToLocalstorage=function(order_id){
        if(!!window.localStorage){//查询条件处理
            var err_temp_book_order_str = localStorage.getItem('err_temp_book_order_'+order_id);
            if(!err_temp_book_order_str)
                return;
    ...
    var saveOrderToLocalstorage=function(order_dto){
        if(!!window.localStorage){
            localStorage.setItem("err_temp_book_order_"+order_dto.id, JSON.stringify(order_dto));
        }
    };
    ...
    $('#land_sendTruck').click(function(){
        ...
        $.post('/bookOrder/sendTruckorder', {order_id:order_id}, function(data){
