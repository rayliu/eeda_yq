...
//        eeda.bindTableLocationField('land_table','ROUTE_TO');
    ...
    //------------事件处理
	 var cargoTable = eeda.dt({
            ...
            { "data": "DOC_NAME", "width": "180px",
            	"render": function ( data, type, full, meta ) {
            		if(!data){
            			...
            			var arr = data.split(",");
            			var idStr = full.BOOK_ORDER_LAND_DOC_ID;
            ...
            { "data": "BOOK_ORDER_LAND_DOC_ID", "visible": false,
            	"render": function ( data, type, full, meta ) {
            		if(!data)
            			data='';
            		return data;
    ...
    itemOrder.refleshLandItemTable = function(order_id){
    	var url = "/bookOrder/tableList?order_id="+order_id+"&type=land";
  ...
  //checkbox选中则button可点击
    ...
    $("#land_table").on('click', '.delete_sign_desc', function(){
    	...
    	var id = tr.attr('id');
    	var order_id = $('#order_id').val();
	     $.post('/bookOrder/deleteSignDesc', {id:id}, function(data){
    ...
    $("#land_table").on('click', '.delete_icon_of_sign_desc', function(){
    	...
    	var id = $(this).attr('id');
    	var order_id = $('#order_id').val();
	     $.post('/bookOrder/deleteOneSignDesc', {id:id,name:name}, function(data){
	        	 $.scojs_message('删除成功', $.scojs_message.TYPE_OK);
	        	 itemOrder.refleshLandItemTable(order_id);
	     },'json').fail(function() {
	         	 $.scojs_message('删除失败', $.scojs_message.TYPE_ERROR);
    ...
    $("#land_table").on('click', '.upload', function(){
		...
		var order_id = $('#order_id').val();
			$(this).fileupload({
				autoUpload: true, 
			    url: '/bookOrder/uploadSignDesc?id='+id,
			    dataType: 'json',
