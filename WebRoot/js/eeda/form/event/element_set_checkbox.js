define(['jquery'], function ($) {
    console.log('enter element_set_checkbox.js');
    ...
    var handle = function(action){
        ...
        for (let index = 0; index < selected_nodes.length; index++) {
            ...
            var type=node.type;
            var id = node.id;
            var checkbox_val = node.checkbox_val;
            var element_id = "[name^=form_"+form_id+"-f"+id+"]";
            var $checkboxs = $(element_id);

            var val_arr=checkbox_val.split(",");
            for (let index = 0; index < val_arr.length; index++) {
                const select_value = val_arr[index];
                $checkboxs.filter('[value='+select_value+']').prop('checked', true);
            }
        }
    }
