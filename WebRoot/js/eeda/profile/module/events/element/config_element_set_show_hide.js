define(['jquery'], function ($) {
    ...
    var redisplay= function(node){
        ...
        if(event_action_setting){
            ...
            eventModuleTreeObj = $.fn.zTree.init($("#config_element_set_show_tree"), setting, nodes);
    ...
    function addDiyDom(treeId, treeNode) {
        ...
        if(treeNode.radio_val&&treeNode.radio_val=='show'){
            enable_checked="checked";
        }else{
            disable_checked="checked";
        }
        var selectStr = '<label style="margin-left:5px;"><input type="radio" class="sel" name="config_show_field_'+treeNode.id+'" value="hide" '+disable_checked+'>隐藏</label>'+
                ' <label><input type="radio" class="sel" name="config_show_field_'+treeNode.id+'" value="show" '+enable_checked+'>显示</label>';
    ...
    var loadFields = function(){
        ...
        for (var i = 0; i < module_obj.BTN_LIST_EDIT.length; i++) {
            ...
            defaultNodes[0].children=edit_btn_children;
        }
        for (var i = 0; i < module_obj.FORM_FIELDS.length; i++) {
            ...
            defaultNodes[1].children=fields_children;
        }

        eventModuleTreeObj = $.fn.zTree.init($("#config_element_set_show_tree"), setting, defaultNodes);
        eventModuleTreeObj.expandAll(true);
    };
    
    
    $('#config_element_set_show_tree').on('click', 'label, .button.chk', function(){
        var tagName = $(this).prop("tagName");
        var treeObj = $.fn.zTree.getZTreeObj("config_element_set_show_tree");
