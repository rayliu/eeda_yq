...
$(document).ready(function() {
    ...
    //费用明细取消确认按钮动作
    $("#charge_table").on('click', '.chargeCancelConfirm_btn', function(e){
    	e.preventDefault();
    	var id = $(this).val();
    	var audit_flag = "N";
    	$.post('/transJobOrder/feeConfirm',{id:id,audit_flag:audit_flag},function(joa){
            ...
            { "width": "80px",
                "render": function ( data, type, full, meta ) {
                	...
                	if(full&&full.AUDIT_FLAG == 'Y'){
                		...
                		str+= '<button type="button" class="btn table_btn chargeCancelConfirm_btn btn_green btn-xs" style="width:50px" value="'+full.ID+'" >取消确认</button> '; 
                		}
                	else if(full.ID){
                		...
                		str+= '<button type="button"  class=" btn table_btn btn_green btn-xs" disabled style="width:50px">取消确认</button> ';	
                	}else{
                		str+= '<button type="button" class="delete btn table_btn delete_btn btn-xs" style="width:50px">删除</button>&nbsp';
                		str+= '<button type="button" class="btn table_btn btn_green btn-xs" style="width:50px"  disabled>确认</button> ';
