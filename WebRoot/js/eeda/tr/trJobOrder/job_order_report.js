...
$(document).ready(function() {
	...
	$('#printOceanWord').click(function(){
    	...
    	$.post('/trJobOrderReport/printOceanWord', {order_id:order_id}, function(data){
    		if(data){
                window.open(data);
    		  }else{
	...
	$('#printOceanSI').click(function(){
		...
		if(alert!=''){
			...
			$('#pdfAlert').click();
		    	...
		    	var order_id = $("#order_id").val();
		    	$.post('/trJobOrderReport/printOceanSI', {order_id:order_id}, function(data){
		    		if(data){
		                window.open(data);
		                $.post('/trJobOrder/siflag', {order_id:order_id}, function(data){
		...
		if(alert!=''){
			...
			$('#pdfAlert').click();
		    	...
		    	var order_id = $("#order_id").val();
		    	$.post('/trJobOrderReport/printOceanHBLSI', {order_id:order_id}, function(data){
		    		if(data){
		                window.open(data);
		                $.post('/trJobOrder/siflag', {order_id:order_id}, function(data){
    ...
    $('#printOceanHBL,#prinTelextOceanHBL').click(function(){
		...
		if(alert!=''){
	    	...
	    	$.post('/trJobOrderReport/printOceanHBL', {order_id:order_id,hbl_no:hbl_no,printHBL:printHBL}, function(data){
	    		if(data){
	                window.open(data);
	             }else{
    ...
    $('#printOceanBooking').click(function(){
    	...
    	if(alert!=''){
	    	...
	    	$.post('/trJobOrderReport/printOceanBooking', {order_no:order_no}, function(data){
	    		if(data){
	    			window.open(data);
	    		}else{
...
//			   });
    ...
    $('#printOceanHead').click(function(){
		...
		$.post('/trJobOrderReport/printOceanHead', {params:JSON.stringify(oceanHead)}, function(data){
    ...
    $('#oceanMBL').click(function(){
    	var order_id = $("#order_id").val();
    	$.post('/trJobOrder/mblflag',{order_id:order_id},function(data){
    		if(data.result==true){
    ...
    $('#alreadyAFR_AMS').click(function(){
    	var order_id = $("#order_id").val();
    	$.post('/trJobOrder/aframsflag',{order_id:order_id},function(data){
    		if(data.result==true){
    ...
    $('#alreadyInline').click(function(){
    	var order_id = $("#order_id").val();
    	$.post('/trJobOrder/alreadyInlineFlag',{order_id:order_id},function(data){
    		if(data.result==true){
    ...
    $('#printAirBooking').click(function(){
    	    	...
    	    	if(alert!=''){
		    		...
		    		var order_no = $("#order_no").val();
		    		$.post('/trJobOrderReport/printAirBooking', {order_no:order_no}, function(data){
		    			if(data){
		    				window.open(data);
		    			}else{
	    	...
	    	if(alert!=''){
				...
				$('#land_table input[type="checkbox"]').each(function(){
					...
					if(checkbox){
						...
						k++;
				    	$.post('/trJobOrderReport/printTruckOrderPDF', {itemId:itemId}, function(data){
							if(data){
								 window.open(data);	
								 $.post('/trJobOrder/truckOrderflag', {itemId:itemId,order_id:order_id}, function(data){
...
//       		itemCurrencyArray.splice($.inArray(currencyArray[i],itemCurrencyArray),1);  移除数组元素
	...
	$('#printDebitNoteBtn').click(function(){
		$(this).attr('disabled', true);
	    	...
	    	var order_type = $('#type').val();
	    	 ...
	    	 $.post('/trJobOrder/saveDebitNote', {itemIds:itemIdsStr,invoiceNo:invoiceNo,accountIdArray:accountIdArrayStr,order_id:order_id}, function(data){
	    		 if(data.result==true){
			    	$.post('/jobOrderReport/printTrDebitNotePDF', {debit_note:debit_note, itemIds:itemIdsStr,accountIdArray:accountIdArrayStr,order_no:order_no,order_type:order_type}, function(data){
	...
	$('#land_printDebit .btn-primary').click(function(){
		...
		$.post('/trJobOrder/saveDebitNoteOfLand', {landIds:landIdsStr,invoice_land_hbl_no:invoice_land_hbl_no,land_ref_no:land_ref_no}, function(data){
			if(data.result==true){
				$.post('/trJobOrderReport/printDebitNotePDF', {debit_note:'land_invoice', landIds:landIdsStr}, function(data){
    ...
    $('#printCabinetTruck').click(function(){
		...
		$('#land_table input[type="checkbox"]:checked').each(function(){
				...
				k++;
				$.post('/trJobOrderReport/printCabinetTruck', {params:JSON.stringify(truckHead)}, function(data){
