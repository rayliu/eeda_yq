...
$(document).ready(function() {
	...
	$('#createCustomPlanOrderBtn').click(function(event) {
        ...
        if(custom_broker.trim()==''){
        	...
        	if(id!='' && custom_type.split(',')[0]=='china'){
            	window.open("/customPlanOrder/create?jobOrderId="+id+"&login_id="+login_id+"&to_office_id="+custom_broker, '_blank');
            }else{
	...
	var showServiceTab=function(service){
        if(service=='china'){
            ...
            $('#entrust_or_self_custom').find('input[name=entrust_or_self_custom]:eq(0)').attr('checked','true');
...
//    
    ...
    var customTable = eeda.dt({
     ...
     itemOrder.refleshCustomAPPTable = function(order_id){
    	var url = "/trJobOrder/tableList?order_id="+order_id+"&type=custom_app";
