...
$(document).ready(function() {
	...
	$('#fileuploadOfCustom').click(function(){
		...
		$('#fileuploadOfCustom').fileupload({
				autoUpload: true, 
			    url: '/trJobOrder/uploadCustomDoc?order_id='+order_id,
			    dataType: 'json',
    ...
    $("#custom_doc_table").on('click', '.delete', function(e){
        ...
        var id = tr.attr('id');
         ...
         $.post('/trJobOrder/deleteCustomDoc', {id:id}, function(data){
        	 if(data.result==true){
    ...
    $('#custom_doc_table').on('change','.checkBox',function(){
    	...
    	$.post('/trJobOrder/updateShare', {order_id:$('#order_id').val(),item_id:item_id,check:check}, function(data){
    		if(data){
    ...
    itemOrder.refleshCustomDocTable = function(order_id){
    	var url = "/trJobOrder/tableList?order_id="+order_id+"&type=custom_doc";
    ...
    $('#allCheckOfCustomDoc').click(function(){
    	...
    	$.post('/trJobOrder/updateShare', {order_id:$('#order_id').val(),check:ischeck}, function(data){
    		if(data){
    			$.scojs_message(msg+'成功', $.scojs_message.TYPE_OK);
    			$.unblockUI();
    		}
