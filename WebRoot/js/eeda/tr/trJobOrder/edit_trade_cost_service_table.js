...
$(document).ready(function() {
    ...
    $('#CostServiceDiv').on('click', '.selectCostTemplate,.selectCostTemplate', function(){
        ...
        var dataTable = $('#'+table).DataTable();
         ...
         for(var i=0; i<li.length; i++){
            ...
            var row = $("#cost_service_table tbody tr");
   ...
   var total_calculation = function(row){
	   ...
	   $($('.dataTables_scrollFoot tr')[2]).find('.amount').html(amount.toFixed(1));
	   $($('.dataTables_scrollFoot tr')[2]).find('.cny_total_amount').html(total_cny_total_amount.toFixed(3));
	   $($('.dataTables_scrollFoot tr')[2]).find('.rmb_difference').html(total_rmb_difference.toFixed(3));
       $($('.dataTables_scrollFoot tr')[2]).find('.exchange_total_amount_rmb').html(total_exchange_total_amount_rmb.toFixed(3));
   }
    ...
    //保存应付服务费用模板
    $('#costServiceBtnTemplet').click(function(){
    	var costServiceOrderTemplet={};
    	costServiceOrderTemplet.order_type = $('#type').val();
    	costServiceOrderTemplet.customer_id = $('#customer_id').val();
    	costServiceOrderTemplet.costService_template = itemOrder.buildCostServiceTemplate();
    	costServiceOrderTemplet.allCostService_template = itemOrder.buildAllCostServiceTemplate();
    	$.post('/trJobOrder/saveCostServiceTemplet',{params:JSON.stringify(costServiceOrderTemplet)},function(data){
    		if(data){
    			$.scojs_message('应付服务费用模板保存成功', $.scojs_message.TYPE_OK);
    		}else{
    			$.scojs_message('应付服务费用模板保存失败', $.scojs_message.TYPE_ERROR);
    		}
    	});
    });
    
  //校验
    $('#cost_service_table').on("blur","[name=price],[name=amount],[name=exchange_rate],[name=exchange_currency_rate],[name=exchange_currency_rate_rmb],"
    ...
    $('#cost_service_table').on("focus","[name=price],[name=amount],[name=exchange_rate],[name=exchange_currency_rate],[name=exchange_currency_rate_rmb],"
    		+"[name=remark]",function(){
