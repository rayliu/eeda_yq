...
$(document).ready(function() {
  ...
  //默认填上的字段
	...
	$("#createSave").on('click',function(){
		...
		order.ids=$('#ids').val();
		$.post('/tradeCostRequest/save',{params:JSON.stringify(order)}, function(data){
			// $("#createSave").attr("disabled", false);
			if(data.ID>0){
				$.scojs_message('创建新申请成功', $.scojs_message.TYPE_OK);
				$("#order_no").val(data.ORDER_NO);
				if(confirm('刚生成的新申请单号'+data.ORDER_NO+':是否前往该申请单？')){
					self.location='/tradeCostRequest/edit?id='+data.ID; 
				 }
...
//			}
			...
			$.get("/tradeCostRequest/confirmOrder", {params:JSON.stringify(order)}, function(data){
				if(data){
					$("#status").val('已收款');
					$("#returnBtn").attr("disabled", true);
