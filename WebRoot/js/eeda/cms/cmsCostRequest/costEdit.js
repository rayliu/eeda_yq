...
//    	$('#receive_time').val(eeda.getDate());
	...
	$("#saveBtn").on('click',function(){
		...
		$.get('/cmsCostRequest/save',{params:JSON.stringify(order)}, function(data){
    ...
    //打印应付申请单
	 ...
	 //复核
	  $("#checkBtn,#cancelcheckBtn").on('click',function(){
			...
			$.get("/cmsCostRequest/checkOrder", {order_id:$('#order_id').val(),selfId:selfId,order_no:order_no,creator_name:creator_name,invoice_no:invoice_no}, function(data){
				if(data.ID>0){
...
//			}
			...
			$.get("/cmsCostRequest/confirmOrder", {params:JSON.stringify(order)}, function(data){
				if(data){
					$("#status").val('已付款');
					$("#returnBtn").attr("disabled", true);
