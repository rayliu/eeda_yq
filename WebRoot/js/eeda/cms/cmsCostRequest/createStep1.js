define(['jquery', 'metisMenu', 'sb_admin', './costEdit_select_item', ,  'dataTablesBootstrap', 'validate_cn', 'sco'], function ($, metisMenu,sb,selectContr) {
	...
	var costAccept_table = eeda.dt({
	    ...
	    columns: [
				...
				{	"width":"70px",
					"render": function(data, type, full, meta) {
						return eeda.numFormat(parseFloat(full.TOTAL_AMOUNT - full.PAID_CNY).toFixed(2),3);	
					}
				}
        ]      
    });
	...
	var searchData=function(){
        ...
        var url = "/cmsCostRequest/OrderList?sp_id="+sp
      	   +"&order_no="+order_no
           +"&status="+status
    ...
    var refleshSelectTable = function(){
	    var url = "/cmsCostRequest/OrderList";
	    costAccept_table.ajax.url(url).load();
    }
	//点击时，赋值给sp_id_input
	var sp_name='';
