...
$(document).ready(function() {
    ...
    $('#saveBtn').click(function(e){
        ...
        $.post('/cmsCostCheckOrder/save', {params:JSON.stringify(order)}, function(data){
            ...
            if(order.ID>0){
        		...
        		$('#add_cost,.delete,.itemEdit').attr('disabled', false);
                //异步刷新明细表
    ...
    if(order_id==""){
    	$('#saveBtn').attr('disabled', false);
    	$('#add_cost,.delete,.itemEdit').attr('disabled', true);
    }else{
    	if(status=='新建'){
    		$('#saveBtn,#refuseBtn,#confrimBtn').attr('disabled', false);
    		$('#add_cost,.delete,.itemEdit').attr('disabled', false);
    ...
    $("#cancelConfirmBtn").click(function(){
		var id = $("#order_id").val();
		$.post("/cmsCostCheckOrder/cancelConfirm",{id:id},function(data){
			if(data){
				$("#status").val('取消确认');
				$.scojs_message('取消确认成功', $.scojs_message.TYPE_OK);
				$('.itemEdit,.delete,#add_cost,#confrimBtn,#refuseBtn,#saveBtn').attr('disabled', false);
	    		$('#cancelConfirmBtn').attr("disabled",true);
			}else{
