...
//	}
	...
	$("#saveBtn").on('click',function(){
		...
		order.ids=$('#ids').val();
		$.post('/cmsChargeRequest/save',{params:JSON.stringify(order)}, function(data){
    ...
    //打印
	 ...
	 //复核
	  $("#checkBtn").on('click',function(){
			...
			$.get("/cmsChargeRequest/checkOrder", {order_id:$('#order_id').val(),}, function(data){
				if(data.ID>0){
...
//			}
			...
			$.get("/cmsChargeRequest/confirmOrder", {params:JSON.stringify(order)}, function(data){
				if(data){
					$("#status").val('已收款');
					$("#returnBtn").attr("disabled", true);
