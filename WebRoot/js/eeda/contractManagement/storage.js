define(['jquery', 'metisMenu', 'template', 'sb_admin',  'dataTablesBootstrap', 'validate_cn', 'sco', 'jq_blockui'], function ($, metisMenu, template) { 
	$(document).ready(function() {
	    ...
	    //删除一行
	    $("#storage_table").on('click', '.delete', function(e){
	        e.preventDefault();
	    ...
	    itemOrder.buildStorageDetail=function(){
	    	var cargo_table_rows = $("#storage_table tr");
            ...
            columns:[
            	...
            	{ "data": "INFRASTRUCTURE", 
	            	"render": function ( data, type, full, meta ) {
	            		if(!data)
	            			data='';
	            		return '<input type="text" name="infrastructure"  value="'+data+'" class="form-control" />';
	            	}
            	},
            	{ "data": "REMARK", 
	            	"render": function ( data, type, full, meta ) {
	    ...
	    $('#add_storage_item').on('click', function(){
	        var item={};
	        cargoTable.row.add(item).draw(true);
	    });
	    
	    //刷新明细表
	    itemOrder.refleshStorageTable = function(order_id){
	    	var url = "/supplierContract/tableList?order_id="+order_id+"&type=storage";
