define(['jquery', 'metisMenu', 'template', 'sb_admin',  'dataTablesBootstrap', 'validate_cn', 'sco'], function ($, metisMenu, template) {
    $(document).ready(function() {
        ...
        itemOrder.refleshOceanLocTable = function(contract_id){
        	var url = "/supplierContract/tableList?contract_id="+contract_id+"&type=ocean_loc";
        ...
        $('#ocean_location_table').on('click','[name=selectRoute]', function(){
            ...
            var url = "/supplierContract/clickItem?contract_id="+order_id+"&type=ocean&supplier_loc_id="+item_id;
        	cargoTable.ajax.url(url).load();
        });
        ...
        $('#ocean_location_table').on('blur','[name=remark]',function(){
        	var data = $(this).val();
        	var len = $.trim(data).length;
        	var re = /^.{255,}$/;
        	if(re.test(data)&&len>0){
        		$(this).parent().append("<span style='color:red;display:block;' class='error_span'>最多能输入长度为255的字符串</span>")
        	}
        });
        $('#charge_ocean_table').on('focus','[name=price],[name=volume1],[name=volume2],[name=gross_weight1],[name=gross_weight2]',function(){
        	$(this).parent().find("span").remove();
        });
        $('#ocean_location_table').on('focus','[name=remark]',function(){
        	$(this).parent().find("span").remove();
        });
    });
});